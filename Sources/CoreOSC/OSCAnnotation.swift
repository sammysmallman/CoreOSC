...
public struct OSCAnnotation {
    ...
    /// The annotation expressions, compiled once — NSPredicate compiled them
    /// per call, and its format matching is unimplemented on Linux.
    private static let equalsCommaRegex = try! NSRegularExpression(pattern: OSCAnnotationStyle.equalsComma.regex)
    private static let spacesRegex = try! NSRegularExpression(pattern: OSCAnnotationStyle.spaces.regex)

    private static func regex(for style: OSCAnnotationStyle) -> NSRegularExpression {
        switch style {
        case .equalsComma: return equalsCommaRegex
        case .spaces: return spacesRegex
        }
    }

    public static func evaluate(_ annotation: String,
                                style: OSCAnnotationStyle = .spaces) -> Bool {
        regex(for: style).firstMatch(in: annotation, range: annotation.nsrange)?.range == annotation.nsrange
    }
    ...
    private static func equalsCommaMessage(for annotation: String) -> OSCMessage? {
        let matches = regex(for: .equalsComma)
