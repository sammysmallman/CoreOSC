...
The CoreOSC package contains common infrastructural code for your apps to communicate among computers, sound synthesisers, and other multimedia devices via [OSC](http://opensoundcontrol.org/README.html).
...
#### Initialisation with an `OSCAddressPattern`:
```swift
let addressPattern = try! OSCAddressPattern("/core/osc/*")
...
```
#### Initialisation with a raw address pattern `String`:
```swift
...
```
Initialisation of an `OSCMessage` will `throw` if the format is incorrect or invalid characters are found in the given `String` address pattern.

---

### Bundles
An `OSCBundle` is a container for messages, but also other bundles and allows for the invocation of multiple messages atomically as well as scheduling them to be invoked at some point in the future. For further information regarding the temporal semantics of bundles and their associated `OSCTimeTag`s, please see [OSC 1.0](http://opensoundcontrol.org/spec-1_0.html#timetags).
```swift
...
For example:
```swift
let method = OSCMethod(with try! OSCAddress("object/coords")) { [weak self] message, _ in
  guard message.arguments.count == 2,
        case let .float32(x) = message.argument[0], 
        case let .float32(y) = message.argument[1]
  else { return }
...
## Extensions
The following objects are not part of either OSC specification but have been developed after observation of implementations of OSC in the wild and aim to provide help and functionality to you in specific OSC communication scenarios.

### Address Store
An `OSCAddressStore` is a container of `OSCAddress`'s and is a simplified version of an `OSCAddressSpace` without any method invocation. Calling `filter(with:_)` will provide an array of `OSCAddress`'s that would be invoked by the received `OSCMessage`. If you are just looking to provide a network filter with method invocation further up the stack `count(with:_)` can be used to ascertain whether the `OSCMessage` should be filtered out. e.g. A count of 0 would indicate dropping the packet.

For example:
```swift
let address1 = try! OSCAddress("/core/osc/1")
let address2 = try! OSCAddress("/core/osc/2")
let address2 = try! OSCAddress("/core/osc/3")

let store = OSCAddressStore(addresses: [address1, address2, address3])
var message = OSCMessage(raw: "/core/osc/1")

var addresses = store.filter(with: message)
// addresses is equal to [address1].
var count = store.count(with: message)
// count is equal to 1.

message = OSCMessage(raw: "/core/osc/*")

addresses = store.filter(with: message)
// addresses is equal to [address1, address2, address3].
count = store.count(with: message)
// count is equal to 3.
```
...
let annotation1 = OSCAnnotation.annotation(
  for: message,
  style: .spaces,
  type: false
)
...
let annotation2 = OSCAnnotation.annotation(
  for: message,
  style: .spaces,
  type: true
)
...
### Address Filters
An `OSCAddressFilter` is kind of the reverse of an `OSCAddressSpace`. Where an address space allows for an address pattern to invoke multiple pre defined methods. An address filter allows for a single method to be invoked by multiple loosely formatted address patterns by using a "#" wildcard character and omitting parts from the pattern matching. Think of an address filter as a container for blocks of code, that can be dispatched when a message is received, with an address pattern that matches against a filter methods `OSCFilterAddress`.
...
For example:
```swift
let method = OSCFilterMethod(with: try! OSCFilterAddress("cue/#/fired")) { [weak self] message, _ in
...
An `OSCFilterAddressStore` is a container of `OSCFilterAddress`'s and is a simplified version of an `OSCAddressFilter` without any method invocation. Calling `filter(with:_)` will provide an array of sorted `OSCFilterAddress`'s that would be invoked by the received `OSCMessage`. If you are just looking to provide a network filter with method invocation further up the stack `count(with:_)` can be used to ascertain whether the `OSCMessage` should be filtered out. e.g. A count of 0 would indicate dropping the packet.

For example:
```swift
let address1 = try! OSCFilterAddress("/core/osc/#")
let address2 = try! OSCFilterAddress("/core/osc/test/something")

let store = OSCFilterAddressStore(addresses: [address1, address2])
let message = OSCMessage(raw: "/core/osc/1")

let addresses = store.filter(with: message)
// addresses is equal to [address1].

let count = store.count(with: message)
// count is equal to 1.
```
